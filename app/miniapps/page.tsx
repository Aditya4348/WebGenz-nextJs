...
const MiniApps: React.FC = () => {
  ...
  const apps = [
    ...
    },
    {
      ...
      bg: "bg-indigo-50",
      component: AestheticText,
    },
  ];

  const ActiveComponent =
    apps.find((a) => a.id === activeApp)?.component || MoodChecker;
      ...
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto px-4">
                ...
                >
                  <div
                    className={`p-1.5 rounded-full ${
                      activeApp === app.id ? "bg-white/20" : app.bg
                    } ${activeApp === app.id ? "text-white" : app.color}`}
                  >
                    <app.icon size={16} />
                  </div>
        ...
        <div className="grow">
          <div className="bg-white rounded-5xl border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6 md:p-12 min-h-[400px] flex flex-col justify-center relative overflow-hidden transition-all">
            ...
            <div className="relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-500 key={activeApp}">
              <ActiveComponent />
            </div>
          </div>
        </div>
