...
import { Sparkles, ArrowDown, Zap, Star, Code, Coffee, Send, MessageSquarePlus, Hash, Lock, StickyNote, ArrowUpRight, Heart, Smile, Lightbulb, Palette, CheckCircle, Gamepad2 } from 'lucide-react';
...
const Marquee: React.FC = () => (
  <div className="w-full overflow-hidden bg-black py-3 border-y border-black rotate-1 my-12">
    <div className="flex animate-marquee whitespace-nowrap gap-8">
      {[...Array(10)].map((_, i) => (
        <div key={i} className="flex items-center gap-8 text-white font-bold font-display text-lg uppercase tracking-widest">
           <span>Coding</span>
           <Star size={16} className="text-gen-yellow fill-gen-yellow" />
           <span>Overthinking</span>
           <Star size={16} className="text-gen-mint fill-gen-mint" />
           <span>Coffee</span>
           <Star size={16} className="text-gen-pink fill-gen-pink" />
           <span>Memes</span>
           <Star size={16} className="text-gen-blue fill-gen-blue" />
        </div>
      ))}
    </div>
  </div>
...
const Home: React.FC = () => {
  ...
  const trendingPosts = blogPosts.filter(p => p.tags.includes('Life') || p.tags.includes('React')).slice(0, 2);
  const recentMenfess = menfessData.slice(0, 3);

  // Data Preview Mini Apps
  const miniAppsPreview = [
    { id: 'mood', label: 'Mood Checker', icon: Smile, color: 'bg-yellow-300', desc: "Cek vibe lo hari ini." },
    { id: 'inspiration', label: 'Inspo Gen', icon: Lightbulb, color: 'bg-purple-300', desc: "Cari ide random." },
    { id: 'palette', label: 'Palette', icon: Palette, color: 'bg-pink-300', desc: "Warna aesthetic." },
    { id: 'decision', label: 'Decision', icon: CheckCircle, color: 'bg-red-300', desc: "Galau? Klik ini." },
  ];
    ...
    <div className="space-y-24 pb-12 ">
      ...
      <section className="relative pt-8 md:pt-16 pb-8 flex flex-col items-center text-center px-4">
        ...
        <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-white border-2 border-gray-100 shadow-sm mb-8 animate-pulse">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-gray-600">Portorin v2.0 Live</span>
        </div>
        ...
        <h1 className="font-display font-black text-6xl md:text-8xl lg:text-9xl leading-none mb-8 tracking-tighter relative z-10">
          ...
          <span className="relative inline-block px-4">
            <span className="relative z-10 text-white drop-shadow-lg">RANDOM</span>
            <span className="absolute inset-0 bg-black rounded-full rotate-2 transform scale-110 z-0"></span>
          </span> <br/>
          TAPI <span className="text-transparent bg-clip-text bg-linear-to-r from-purple-500 to-pink-500">BERMAKNA.</span>
        </h1>
        
        <p className="text-gray-500 text-xl md:text-2xl max-w-2xl mx-auto leading-relaxed mb-12 font-medium">
          Internet playground buat lo yang butuh asupan insight, 
          tutorial coding, atau sekedar validasi kalau hidup itu emang chaos.
        </p>

        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          <Link href="/blog" className="group px-8 py-4 bg-gen-yellow border-2 border-black text-black rounded-full font-bold shadow-[4px_4px_0px_0px_black] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all flex items-center justify-center gap-2">
             Mulai Baca <ArrowDown size={20} className="group-hover:animate-bounce" />
          </Link>
          <a href="#featured" className="px-8 py-4 bg-white text-black border-2 border-gray-200 rounded-full font-bold hover:bg-gray-50 hover:border-black transition-all">
            Explore Dulu
          </a>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 flex flex-col items-center">
        ...
        <div className="mt-8 w-full max-w-3xl">
            <div className={`bg-white rounded-b-4xl p-3 border-2 shadow-sm transition-all focus-within:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] ${!user ? 'border-gray-200' : 'border-black focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-black'}`}>
               
               {!submitted ? (
                   <form onSubmit={handleSubmitQuote} className="flex flex-col md:flex-row gap-3">
            ...
            <div className="flex justify-between items-center mt-3 px-2">
                ...
                {!user && (
                    <Link href="/login" className="text-xs font-bold text-purple-600 hover:underline">
      ...
      <section id="featured" className="max-w-7xl mx-auto px-4">
        <SectionHeader title="Baru Diupload" subtitle="Tulisan yang masih anget, fresh from the oven." link="/blog" />
      ...
      <section className="max-w-7xl mx-auto px-4 bg-gen-off py-16 sm:bg-transparent sm:py-0">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
              ...
              <Link href="/menfess" className="flex items-center gap-2 px-6 py-3 rounded-full bg-white border border-gray-200 font-bold text-sm hover:bg-black hover:text-white transition-all group">
                 Buka Tembok <ArrowUpRight size={16} className="group-hover:rotate-45 transition-transform" />
              </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recentMenfess.map((m, idx) => (
                  <Link 
                    href="/menfess"
                    key={m.id} 
                    className={`relative block p-6 rounded-4xl border-2 border-black/5 shadow-sm hover:shadow-xl transition-all hover:-translate-y-2 group ${m.color} ${idx % 2 === 0 ? 'md:rotate-1' : 'md:-rotate-1'} hover:rotate-0`}
                  >
                      {/* Pin Decoration */}
                      ...
                      <div className="flex justify-between items-start mb-4 border-b border-black/10 pb-2">
                          <div>
                              ...
                              <p className="font-bold leading-tight text-sm truncate max-w-[100px]">{m.to}</p>
                          </div>
                          <div className="text-right">
                              <p className="text-[10px] font-bold uppercase tracking-wider opacity-50">From:</p>
                              <p className="font-bold leading-tight text-sm truncate max-w-[100px]">{m.from}</p>
                          </div>
                      </div>
                      ...
                      <div className="flex justify-between items-end mt-auto">
                          ...
                          <div className="bg-white/40 px-3 py-1.5 rounded-full flex items-center gap-1.5">
                              ...
                              <span className="text-xs font-bold opacity-60">{m.reactions}</span>
                          </div>
                      </div>
                  </Link>
              ))}
          ...
          <div className="mt-8 text-center md:hidden">
              <Link href="/menfess" className="inline-block border-b-2 border-black font-bold pb-1">Lihat {menfessData.length - 3} pesan lainnya...</Link>
          </div>
      </section>
              ...
              <div className="flex flex-col lg:flex-row items-center gap-12 relative z-10">
                  ...
                  <div className="w-full lg:w-1/3 text-center lg:text-left">
                      ...
                      <p className="text-gray-400 mb-8 font-medium">
                          Kumpulan mini-apps buat seru-seruan, cari inspirasi, atau sekedar buang waktu dengan produktif (dikit).
                      </p>
                      <Link href="/miniapps" className="inline-flex items-center gap-2 bg-white text-black px-6 py-3 rounded-full font-bold hover:scale-105 transition-transform">
                  ...
                  <div className="w-full lg:w-2/3 grid grid-cols-2 md:grid-cols-4 gap-4">
                      {miniAppsPreview.map((app, i) => (
                          ...
                          >
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 shadow-lg ${app.color} text-black group-hover:scale-110 transition-transform`}>
                                  <app.icon size={28} />
                              </div>
                              <h3 className="font-bold text-sm mb-1">{app.label}</h3>
                              <p className="text-[10px] opacity-60 font-medium leading-tight group-hover:opacity-80">{app.desc}</p>
                          </Link>
                      ))}
                  </div>
              </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl">
                ...
                <p className="text-xl font-medium mb-8">
                    Jangan dipendem sendiri. Tulis di sini, biar satu internet tau isi kepala lo. 
                    Dapet fame + validasi instan.
                </p>
                <Link href="/contributor" className="inline-flex items-center gap-3 bg-white text-black px-8 py-4 rounded-full font-bold text-lg border-2 border-black hover:bg-black hover:text-white transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]">
            ...
            <div className="relative">
                <div className="w-48 h-48 md:w-64 md:h-64 bg-white rounded-full border-4 border-black flex items-center justify-center relative z-10">
                    <Star size={80} className="text-black fill-black animate-spin-slow" />
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 bg-black rounded-[3rem] p-8 md:p-16 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
            ...
            <h2 className="font-display font-bold text-4xl md:text-5xl">Lagi Trending</h2>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Large Featured */}
                <Link href={`/blog/${trendingPosts[0].id}`} className="group relative bg-gray-900 rounded-[2.5rem] p-1 border border-white/10 hover:border-white/30 transition-all overflow-hidden h-[400px]">
                    <Image src={trendingPosts[0].coverImage} layout="fill" objectFit="cover" className="opacity-60 group-hover:scale-105 transition-transform duration-700" alt={trendingPosts[0].title} />
                    ...
                    <div className="absolute bottom-0 left-0 p-8 w-full">
                         ...
                         <h3 className="text-3xl font-display font-bold mb-2 leading-tight group-hover:underline decoration-yellow-400">{trendingPosts[0].title}</h3>
                         <p className="text-gray-300 line-clamp-2">{trendingPosts[0].excerpt}</p>
                    </div>
                </Link>
                ...
                <div className="flex flex-col gap-6">
                    {trendingPosts.slice(1).map(post => (
                         <Link key={post.id} href={`/blog/${post.id}`} className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded-[2.5rem] p-8 flex flex-col justify-center transition-all group">
                             <div className="flex justify-between items-start mb-4">
                                <span className="text-4xl font-display font-bold text-white/20">0{post.id}</span>
                    ...
                    <div className="flex-1 bg-linear-to-br from-gen-purple to-purple-600 rounded-[2.5rem] p-8 flex flex-col justify-center items-center text-center text-black">
                        ...
                        <h4 className="font-bold text-xl text-white mb-2">Join The Squad!</h4>
                        <p className="text-white/80 text-sm mb-4">Dapetin update artikel receh. 100% Gratis.</p>
                        <Link href="/contributor" className="bg-black text-white px-6 py-2 rounded-full text-sm font-bold w-full hover:scale-105 transition-transform flex items-center justify-center gap-2">
                           Gabung Komunitas <Star size={12} className="text-yellow-400 fill-yellow-400" />
                        </Link>
                    </div>
                </div>
            </div>
