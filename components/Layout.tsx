      ...
      >
        ...
        <div className="hidden md:flex items-center bg-gray-100/50 p-1.5 rounded-full border border-white/50 ml-auto md:ml-0">
          ...
          >
            ...
            {/* Dropdown Menu */}
            <div
              className={`absolute top-full right-0 pt-4 w-64 transition-all duration-200 ${
                isAppsOpen
            ...
            >
              <div className="bg-white rounded-3xl p-2 shadow-xl border border-gray-100 overflow-hidden">
                {miniApps.map((app) => (
                  ...
                  >
                    <div
                      className={`p-2 rounded-full ${app.bg} ${app.color} group-hover:scale-110 transition-transform`}
                    >
                      <app.icon size={16} />
                    </div>
        ...
        <div className="hidden md:block ml-4">
          {!user ? (
            <Link
              href="/login"
              className="bg-black text-white px-6 py-3 rounded-full font-bold text-sm hover:scale-105 active:scale-95 transition-transform flex items-center gap-2"
            >
              Login <LogIn size={16} />
            </Link>
          ) : (
        ...
        {isOpen && (
          <div className="absolute top-24 left-0 w-full bg-white/95 backdrop-blur-xl border border-gray-200 shadow-2xl rounded-4xl p-4 md:hidden flex flex-col gap-2 animate-in slide-in-from-top-4 duration-300 max-h-[80vh] overflow-y-auto">
            ...
            {user && (
              <Link
                href="/profile"
                className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100 mb-2 active:bg-gray-100"
              >
                <img
                  src={user.avatar}
                  alt="Profile"
              ...
              {isMobileAppsOpen && (
                <div className="grid grid-cols-2 gap-2 p-2 pt-0">
                  {miniApps.map((app) => (
                    ...
                    >
                      <div
                        className={`p-2 rounded-full ${app.bg} ${app.color}`}
                      >
                        <app.icon size={18} />
                      </div>
            ...
            <Link
              href="/contact"
              className="p-4 rounded-2xl text-center font-bold text-lg bg-gray-50 text-gray-500 border-2 border-transparent hover:border-black/10 mt-2"
            >
              Contact
            </Link>
            ...
            {!user ? (
              <Link
                href="/login"
                className="p-4 rounded-2xl text-center font-bold text-lg bg-black text-white mt-2 flex items-center justify-center gap-2"
              >
  ...
  return (
    <footer className="mt-20 border-t border-gray-200 bg-white relative overflow-hidden">
      ...
      <div className="absolute -bottom-10 left-0 w-full overflow-hidden opacity-5 pointer-events-none">
        <h1 className="text-[12rem] font-display font-bold whitespace-nowrap leading-none text-center">
          VibeHub
        </h1>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
        <div className="flex flex-col items-start">
          ...
          <p className="text-gray-500 font-medium max-w-xs">
            Digital playground buat Gen Z yang lagi cari validasi hidup,
            tutorial ngoding, atau sekedar meme kucing.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            <a
              href="#"
              className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-black hover:text-white transition-all hover:-translate-y-1"
            >
              IG
            </a>
            <a
              href="#"
              className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-blue-400 hover:text-white transition-all hover:-translate-y-1"
            >
              TW
            </a>
            <a
              href="#"
              className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all hover:-translate-y-1"
            >
              YT
            </a>
          </div>
        ...
        <div className="text-gray-400 text-sm font-medium border-t md:border-t-0 border-gray-100 pt-8 md:pt-0 w-full md:w-auto">
          &copy; {new Date().getFullYear()} VibeHub Media.
          <br />
          Made with{" "}
          <Heart
            size={14}
            className="inline text-red-500 fill-red-500 mx-1 animate-pulse"
          />{" "}
          by TORIN
        </div>
      </div>
